...
# --------------------------------------
# PASS 1: Close any active rows whose attributes changed
# --------------------------------------
# Match only the active target rows whose end_date IS NULL with the same business key
# If any tracked column differs, set end_date to end_timestamp to retire that version

dt.alias('t')\
    .merge(
        source = df.alias('s')\
        condition = 't.location_id = s.location_id AND t.end_date IS NULL AND (t.borough != s.borogh OR t.zone != s.zone OR t.service_zone != s.service_zone)' 
    )\
    .whenMatchedUpdate(
        set = {
...
# Now insert a row for:
    # - keys we just closed in PASS 1, and
    # - brand new keys not present in the target
# We again match on active rows; anything without an active match is inserted

# get the list of ID's that have been closed
insert_id_list = [row.location_id for row in dt.toDF()\
    .filter(f'end_date = 'end_timestamp').select('location_id').collect()]
